...
FRAMES_DIR = Path(os.environ.get("VO_FRAMES_DIR", str(_ROOT / "data" / "frames_15hz")))
OUT_CSV    = Path(os.environ.get("VO_OUT_CSV",    str(_ROOT / "results" / "run_20260623_093123" / "dl_vo_log_full.csv")))
DOWNSAMPLE_SIZE = (640, 480)
MAX_KP_VO    = int(os.environ.get("VO_MAX_KP", "1024"))
EMPTY_EVERY  = int(os.environ.get("VO_EMPTY_EVERY", "25"))  # clear MPS/CUDA cache every N frames
FLUSH_EVERY  = int(os.environ.get("VO_FLUSH_EVERY", "10"))  # flush CSV to OS every N frames
REPORT_EVERY = 200
...
FIELDS = ["frame_idx", "cum_x", "cum_y", "cum_yaw",
...
def main():
    ...
    # Open CSV immediately and flush each row so the dashboard updates live
    csv_fh = open(OUT_CSV, "w", newline="")
    wtr = csv.DictWriter(csv_fh, fieldnames=FIELDS)
    wtr.writeheader()
    csv_fh.flush()

    # Write frame 0 (reference frame, no motion)
    wtr.writerow({"frame_idx": 0, "cum_x": 0.0, "cum_y": 0.0, "cum_yaw": 0.0,
    ...
    for i in range(total):
        ...
        if H is not None:
            ...
            dx = dy = dyaw = 0.0
            ratio = 0.0
        # Rotate per-frame image-plane translation into a fixed world frame
        # by the heading accumulated so far, then integrate. Summing (dx, dy)
        # in the rotating camera frame collapses the path and produces phantom drift.
        ...
        wtr.writerow({"frame_idx": i + 1, "cum_x": cum_x, "cum_y": cum_y, "cum_yaw": cum_yaw,
    ...
    csv_fh.close()
    OUT_CSV.with_suffix(".done").touch()  # signals sat_anchors --watch-vo to finish
